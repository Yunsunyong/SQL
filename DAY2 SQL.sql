...
/*
오라클 자료형
 - NUMBER (정수, 실수숫자), VARCHAR2(가변길이 문자열(4000바이트)),
    CHAR(고정길이 문자열(2000바이트)), DATE(날짜, 시간), LONG(가변길이문자열(2기가바이트))

 - NUMBER : 기본은 정수형임. 기본자릿수도 7임.
   NUMBER(자릿수) => NUMBER(12) : 정수를 최대 12자리까지 기록할 수 있다.
   NUMBER(최대기록자릿수, 소수점아래 자릿수) : 실수형 값을 기록을 원할때
   => NUMBER(10, 2) : 실수형 값을 최대 10자리까지 기록
                            수숫점 아래 자릿수는 2째자리까지 기록가능
                            
 - CHAR(지정자릿수) : 지정한 글자바이트 사이즈만큼의 글자를 기록함.
    CHAR(10) : 무조건 10바이트 기록함.
                   만약, 5바憐만 기록하면 나머지 바이트는 공백문자로 채움.
                   
 - VARCHAR2(최대기록글자바이트수) : 지정한 바이트사이즈 이하의 글자를 기록함.
   VARCHAR2(15) : 최대 15바이트 이내의 글자로 기록함.
                        10바이트 기록하면 그대로 10바이트가  글자크기임.(나머지를 공백으로 안채움)
 -DATE : 날짜와 시간 처리용
...
-- 날짜 포맷 확인
SELECT SYSDATE
FROM DUAL; --오늘 날짜포맷 확인

--오라클 데이터베이스 환경설정 변수 값 확인하기
SELECT * 
FROM V$NLS_PARAMETERS;

-- 한글 바이트 크기 확인하기(LENGTH : 글자 수, LENGTHB : 글자 바이트 수)
...
SELECT SYSDATE - HIRE_DATE  --오늘 - 입사일(근무일수)
FROM EMPLOYEE;
...
-- 사용 3
-- 직원 테이블에서 한선기 팀장(사번이 100인)의 관리를 받고있는 직원 조회
-- 사번, 이름, 전화번호, 부서코드 조회
...
WHERE dept_id = '50'
ORDER BY emp_name ASC;  --오름차순 정렬

-- SELECT 구문 형식 ***************************************************************************************************
/*
기본구문
SELECT *(테이블이 가지고 있는 컬럼 다) | [DISTINCT](컬럼의 기록된 값들 중에서 같은 값을 중복제거) 조회할 컬럼명 | 계산식 [AS] 별칭(ALIAS)
FROM 조회에 사용할 테이블명;
*/

-- * : 모든 컬럼 (테이블이 가진 컬럼 전부 다)
SELECT *
FROM EMPLOYEE;  -- RESULT SET 결과집합
...
-- 계산식에 함수 사용할 수 있음
-- 컬럼의 값이 NULL 인 경우 다른 값으로 바꾸는 함수 : NVL(NULL VALUE)
-- NVL(컬럼명, 바꿀값)
...
-- 별칭에 반드시 "별칭"로 묶어야 하는 경우 : 별칭에 공백이나 숫자, 기호문자 사용될때
SELECT EMP_ID AS 사번, EMP_NAME AS 이름, SALARY "급여(원)", SALARY * 12 AS "  1년급여"
          , BONUS_PCT "보너스 포인트", (SALARY +  (SALARY * NVL(BONUS_PCT,0))) * 12 "보너스포인트적용연봉"
FROM EMPLOYEE;
...
3 : SELECT 컬럼명 별칭, .......
1 : FROM 테이블명
2 : WHERE 컬러명 비교연산자 비교값

- 반드시 FROM 절 다음에 사용해야 함
- 조건절임 : 테이블에서 조건과 일치하는 값을 가진 행들을 골라냄
- 비교연산자 : >(크냐, 초과), <(작으냐, 미만), >=(크거나 같으냐, 이상), <= (작거나 같으냐, 이하), = (같으냐, 일치하는), !=,<>,^= (일치하지 않는, 같지 않은)
...
-- '날짜형식' 묶어서 표기해야 함
SELECT EMP_ID 사번, EMP_NAME 이름, HIRE_DATE 입사일, DEPT_ID
FROM EMPLOYEE
--WHERE HIRE_DATE >= '95/1/1' AND HIRE_DATE <= '00/12/31';
WHERE HIRE_DATE BETWEEN '95/1/1' AND '00/12/31';
...
WHERE DEPT_ID IN ('20','90');   -- ()안에 있는 값을 포함하고 있냐

-- 연결 연산자 : || (자바의 문자열합치기하는 + 와 같은 의미의 연산자)
...
-- 기혼인 직원의 이름, 입사일, 직급코드, 부서코드, 급여, 결혼여부 조회, 별칭
-- 급여값 뒤에는 '(원)' 리터럴 붙여줌
-- 결혼여부는 리터럴을 채움 : '기혼' 이 출력되게 적용함
SELECT EMP_NAME 이름, HIRE_DATE 입사일, JOB_ID 직급코드, DEPT_ID 부서코드, SALARY || '(원)' AS 급여, '기혼' 결혼여부
FROM EMPLOYEE
WHERE HIRE_DATE >= '00/1/1' AND MARRIAGE = 'Y'; --한선기 입사일은 2090/04/01 임
...
-- 이름에 '해' 자가 포함된 직원 정도 조회
-- 이름, 주민번호, 전화번호, 결혼유무, 별칭 처리
...
SELECT EMP_ID 사번, EMP_NAME 이름, EMP_NO 주민번호, PHONE 전화번호, '여자' 성별
FROM EMPLOYEE
WHERE EMP_NO LIKE '_______2%';

-- 남자 직원 명단 조회
SELECT EMP_ID 사번, EMP_NAME 이름, EMP_NO 주민번호, PHONE 전화번호, '남자' 성별
FROM EMPLOYEE
