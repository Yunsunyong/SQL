...
SELECT EMP_ID 사번,
          ...
          BONUS_PCT 보너스포인트
FROM EMPLOYEE
WHERE HIRE_DATE > '82/1/1' OR JOB_ID = 'J3';
...
-- 단일행함수 : 읽은 값이 N개이면, 리턴되는 결과값도 N개임 (EX 10개 읽으면 10개 결과값 리턴)
...
FROM EMPLOYEE
WHERE SALARY > AVG(SALARY);   --ERROR HAVING절에서 처리가능
...
-- LENGTH() 함수를 이용해서, CHAR 자료형과 VARCHAR2 자료형 차이점 비교확인
-- 한글 한글자는 3바이트
...
-- LPAD / RPAD('문자열값' | 컬럼명, 출력시킬 너비지정[, 남은영역에 채울 문자])
...
SELECT EMP_NAME 이름, 
          EMAIL 이메일,
          SUBSTR(EMAIL, 1, INSTR(EMAIL, '@') - 1) 아아디
FROM EMPLOYEE;
...
SELECT HIRE_DATE,ABS(FLOOR(HIRE_DATE - SYSDATE)) 총근무일수,
          ABS(FLOOR(SYSDATE - HIRE_DATE)) 총근무일수
FROM EMPLOYEE;

-- MOD (나눌 값, 나눌 수)
-- 나누기한 나머지 구하는 함수
SELECT FLOOR(25 / 7) 몫, MOD(25,7)
FROM DUAL;
