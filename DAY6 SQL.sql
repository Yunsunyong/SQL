...
-- 사원명, 부서명 조회
SELECT EMP_NAME 사원명, D.DEPT_ID, DEPT_NAME 부서명
FROM EMPLOYEE E, DEPARTMENT D
WHERE E.DEPT_ID = D.DEPT_ID;
...
SELECT *
FROM EMPLOYEE
JOIN DEPARTMENT USING (DEPT_ID);   --오라클 전용구문과는 다르게 공통으로 사용된 컬럼이 맨 앞에 표시됨(비교한 두 컬럼이 하나의 컬럼으로 합쳐짐)
...
SELECT *
FROM DEPARTMENT
JOIN LOCATION ON (LOC_ID = LOCATION_ID);  --오라클 전용구문과 같음
...
-- 오라클 전용구문
SELECT EMP_ID 사번, EMP_NAME 사원명, JOB_TITLE 직급명
...
SELECT *
FROM EMPLOYEE
--LEFT OUTER JOIN DEPARTMENT USING(DEPT_ID); OUTER는 생략가능
...
-- NATURAL [INNER] JOIN    (오라클 전용구문에서는 없음)
...
SELECT E.EMP_NAME 직원이름, M.EMP_NAME "관리자 이름" 
FROM EMPLOYEE E
JOIN EMPLOYEE M ON (E.MGR_ID = M.EMP_ID);
...
SELECT TO_CHAR(TO_DATE('20201225', 'RRRRMMDD'),'DAY') "2020/12/25요일"
FROM DUAL;
...
JOIN JOB USING (JOB_ID)
JOIN DEPARTMENT USING (DEPT_ID)
WHERE TO_CHAR(TO_DATE(SUBSTR(EMP_NO,1,2),'YY'),'YY') >= '60' AND TO_CHAR(TO_DATE(SUBSTR(EMP_NO,1,2),'YY'),'YY') <= '69'
          AND EMP_NO LIKE '_______2%' AND EMP_NAME LIKE '김%';
...
-- 오라클 전용구문
SELECT EMP_ID, EMP_NAME, MIN(TRUNC(MONTHS_BETWEEN(SYSDATE,
          TO_DATE(SUBSTR(EMP_NO,1,2),'RR'))/12)) 나이, DEPT_NAME 부서, JOB_TITLE 직급
...
--오라클 전용구문
SELECT EMP_NAME 사원, BONUS_PCT 보너스포인트, DEPT_NAME 부서명, LOC_DESCRIBE 근무지역명
FROM EMPLOYEE E, DEPARTMENT D, LOCATION L
WHERE E.DEPT_ID = D.DEPT_ID AND
          D.LOC_ID = L.LOCATION_ID AND
          BONUS_PCT IS NOT NULL AND
          BONUS_PCT <> 0.0;
...
JOIN DEPARTMENT USING (DEPT_ID )
JOIN LOCATION ON (LOC_ID = LOCATION_ID)
WHERE BONUS_PCT IS NOT NULL AND
          BONUS_PCT ^= 0.0;
...
FROM EMPLOYEE E, JOB J
WHERE  E.JOB_ID = J.JOB_ID AND
(SALARY + (SALARY * NVL(BONUS_PCT,0))) *12 > MIN_SAL;
...
-- 오라클 전용구문
SELECT EMP_NAME 사원, JOB_TITLE 직급, SALARY 급여
FROM EMPLOYEE E, JOB J
WHERE E.JOB_ID = J.JOB_ID AND
          BONUS_PCT IS NULL AND 
          E.JOB_ID IN ('J4', 'J7');
-- ANSI 표준구문
SELECT EMP_NAME 사원, JOB_TITLE 직급, SALARY 급여
FROM EMPLOYEE
